...
function synchyInstallerReplaceValue($value, string $search, string $replace, bool &$changed)
{
	if (is_string($value)) {
		...
		$updated = synchyInstallerReplaceLocalUrlOrigins($updated, $replace, $changed);
...
function synchyInstallerIsLocalUrlHost(string $host): bool
{
...
function synchyInstallerReplaceLocalUrlOrigins(string $value, string $replace, bool &$changed): string
{
	$replace = synchyInstallerNormalizeUrl($replace);
	...
	foreach ($patterns as $pattern => $replacement) {
		$updated = (string) preg_replace_callback(
			...
			static function (array $matches) use ($replacement, &$changed): string {
				...
				if (!synchyInstallerIsLocalUrlHost($host)) {
