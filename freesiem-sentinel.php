...
define('FREESIEM_SENTINEL_VERSION', '1.0.146');
...
// Local freeSIEM Core (docker stack on the developer's machine), used instead of
// production when this site runs on a *.ddev.site host. `localhost` would be the
// DDEV web container itself, so the Docker host alias is what actually reaches
// the core stack's nginx (self-signed cert, published on 8443).
define('FREESIEM_SENTINEL_LOCAL_BACKEND_URL', 'https://host.docker.internal:8443');
...
require_once FREESIEM_SENTINEL_PLUGIN_DIR . 'includes/class-inventory.php';
...
require_once FREESIEM_SENTINEL_PLUGIN_DIR . 'includes/class-threat-signatures.php';
...
require_once FREESIEM_SENTINEL_PLUGIN_DIR . 'includes/class-cron-monitor.php';
...
require_once FREESIEM_SENTINEL_PLUGIN_DIR . 'includes/class-freesiem-sentinel-s3-client.php';
